...
void DrawBinaryGates(Ui* ui, App* app, Circuit* cir, Wire wires[], BinaryGate binary[])
{
  ...
  binary[cir->gate_count].gate_rec = (Rectangle){
    ...
    cir->snapped_pos.y,
    GRID_SIZE * 4,
    GRID_SIZE + GRID_SIZE * binary[cir->gate_count].inputs
  };
  ...
  if (is_in_workspace)
  {
    ...
    for (int i = 0; i < binary[cir->gate_count].inputs; i++)
    {
      ...
      binary[cir->gate_count].input_points[i] = (Vector2) {cir->snapped_pos.x, cir->snapped_pos.y + i * GRID_SIZE + GRID_SIZE};
...
void RenderBinaryGates(Circuit* cir, BinaryGate binary[])
{
  for (int i = 0; i < cir->gate_count; i++)
  {
    ...
    for (int j = 0; j < binary[i].inputs; j++)
    {
      ...
      binary[i].input_points[j] = (Vector2) {binary[i].gate_rec.x, binary[i].gate_rec.y + j * GRID_SIZE + GRID_SIZE};
